<template>
  <div class="content-body mt-3 d-flex flex-column justify-content-start align-content-start px-3" style="max-width: 1000px">
    ...
    <div class="text-start">
      <span class="fs-4 fw-bold content-title">{{ t("create_kaizen_strategy") }}</span>
    </div>
    ...
    >
      ...
      <div class="mb-5">
        ...
        <!-- 期數 -->
        <el-form-item class="mb-4" :size="formItemSize" prop="periodNum" :label="t('form.label.period')">
          <label class="mx-3 m-sm-0">{{ addForm.project?.periodNum }}</label>
        </el-form-item>
      ...
      <div class="mb-5">
        ...
        <el-form-item class="mb-4" :size="formItemSize" prop="procTimeAfterImp" :label="t('form.label.proc_time_after_improvement')">
          <el-input-number
            ...
            v-model="addForm.procTimeAfterImp"
            :precision="1"
            :step="0.1"
            :min="0.0"
            clearable
        ...
        <el-form-item class="mb-4" :size="formItemSize" prop="hyperLink" :label="t('form.label.hyperLink')">
          ...
          <el-image class="sharepoint-icon mx-2" :src="require('@/assets/sharepoint_2019.svg')" :fit="'cover'" @click="onSharepointClicked"></el-image>
        </el-form-item>
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const procTimeAfterImpValidator = async (rule: any, value: any, callback: any) => {
    ...
    const newKaizenFormRules = ref({
      ...
      procTimeAfterImp: [{ required: true, validator: procTimeAfterImpValidator, trigger: "blur" }],
    });
    ...
    const onSubmitFormClicked = async () => {
      ...
      try {
        ...
        if(createKaizenItemResponse.data.result == APIResultCode.duplicate_kaizen_item) {
          //偵測到重複的項目(同期同一個料號同一個作業編號只允許出現一筆)
          ElNotification({
            title: t("error"),
            message: t("duplicate_kaizen_item"),
            type: "error",
          });
          return;
        }
    ...
    /** 按下Sharepoint連結 */
    const onSharepointClicked = async () => {
      console.log("click");
      const url = "http://tpspap/sites/TPS/Documents/Forms/AllItems.aspx";
